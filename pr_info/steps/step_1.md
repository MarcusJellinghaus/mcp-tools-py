...
- Note: `exists_side_effect` in binary-missing tests must handle both lint-imports and vulture binary paths (return False for both non-python binaries)
...
- `TestCheckToolAvailability.test_lint_imports_unavailable_when_binary_missing` — also assert `"vulture"` key; update `exists_side_effect` to return False for both lint-imports and vulture binary paths
