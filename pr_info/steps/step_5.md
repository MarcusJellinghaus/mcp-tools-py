...
- The module contract is that the first non-empty line is the state header; the timeout
  return must keep that true. Return the header alone — do not prepend the `info_line`.
...
> `timed_out` return before any parsing, and pass
