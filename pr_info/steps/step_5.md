...
**Context:** See `pr_info/steps/summary.md` for full issue context. Steps 1-4 must be completed first.
...
> **Task:** Implement Step 5 of Issue #108 (Add Python refactoring tools).
> Read `pr_info/steps/summary.md` for full context, then follow `pr_info/steps/step_5.md` exactly.
>
...
## Part C: Pytest marker for integration tests

Use the existing `integration` marker (`@pytest.mark.integration`) which is already defined in `pyproject.toml`. Do not create a new custom marker. These tests can be excluded from fast test runs with `-m "not integration"` per CLAUDE.md convention.
...
2. All unit tests from steps 1-4 still pass
