# Step 6: Manual test plan update

> **Context**: See [summary.md](summary.md) for full issue overview.

## Goal

Update the manual test plan to reflect the new `symbol_names` signature and add a
batch move test case.
...
### Update Test 7 — use `symbol_names` parameter
...
**7c-teardown:** Delete `sample_project/` and recreate.

### Update PROGRESS_TRACKER.md

Add rows for Test 7c (dry, apply, verify, teardown).

## LLM PROMPT

```
Implement Step 6 from pr_info/steps/step_6.md (see pr_info/steps/summary.md for context).

Update tests/mcp_tools_py_manual/TEST_PLAN.md:
1. Change all move_symbol calls in Test 7 from symbol_name="X" to symbol_names=["X"]
2. Add Test 7c for batch move (move create_user and is_active together)
3. Update PROGRESS_TRACKER.md template with rows for Test 7c
