...
# Assumes mcp-tools-py is editable-installed (pip install -e .)
...
if ! "$PROJECT_VENV/bin/python" -c "from importlib.metadata import distribution as D; u=D('mcp-tools-py').read_text('direct_url.json') or ''; exit(0 if 'dir_info' in u and 'editable' in u else 1)" 2>/dev/null; then
    echo "WARNING: mcp-tools-py does not appear to be editable-installed from $PWD"
